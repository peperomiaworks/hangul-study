...
const generateLevel3Data = (): HangulChar[] => {
  ...
  */
  ...
  const commonPatchims = [
    { char: 'ㄱ', romaji: 'k', kana: 'ッ' },
    { char: 'ㄴ', romaji: 'n', kana: 'ン' },
    { char: 'ㄹ', romaji: 'l', kana: 'ル' },
    { char: 'ㅁ', romaji: 'm', kana: 'ム' },
    { char: 'ㅂ', romaji: 'p', kana: 'プ' },
    { char: 'ㅇ', romaji: 'ng', kana: 'ン' }
  ];

  // パッチムのUnicodeインデックス
  // 0: なし, 1: ㄱ, 4: ㄴ, 8: ㄹ, 16: ㅁ, 17: ㅂ, 21: ㅇ ... (これは正確ではありません)
  // 正確な順序:
  // 0:なし 1:ㄱ 2:ㄲ 3:ㄳ 4:ㄴ 5:ㄵ 6:ㄶ 7:ㄷ 8:ㄹ 9:ㄺ 10:ㄻ 11:ㄼ 12:ls 13:lt 14:lp 15:lh 16:ㅁ 17:ㅂ 18:bs 19:ㅅ 20:ss 21:ㅇ 22:ㅈ 23:ch 24:k 25:t 26:p 27:h

  const patchimMap: Record<string, number> = {
    'ㄱ': 1, 'ㄴ': 4, 'ㄹ': 8, 'ㅁ': 16, 'ㅂ': 17, 'ㅇ': 21
  };

  basicCombinations.forEach(base => {
    // 一部の文字だけにパッチムを追加 (全てだと多すぎる)
    if (Math.random() > 0.3) return;

    const p = commonPatchims[Math.floor(Math.random() * commonPatchims.length)];
    const pIndex = patchimMap[p.char];
...
const generateKana = (cChar: string, vChar: string): string[] => {
  ...
  const basicMap: Record<string, Record<string, string[]>> = {
    'ㄱ': { 'ㅏ': ['カ', 'ガ'], 'ㅑ': ['キャ', 'ギャ'], 'ㅓ': ['コ', 'ゴ'], 'ㅕ': ['キョ', 'ギョ'], 'ㅗ': ['コ', 'ゴ'], 'ㅛ': ['キョ', 'ギョ'], 'ㅜ': ['ク', 'グ'], 'ㅠ': ['キュ', 'ギュ'], 'ㅡ': ['ク', 'グ'], 'ㅣ': ['キ', 'ギ'] },
    'ㄴ': { 'ㅏ': ['ナ'], 'ㅑ': ['ニャ'], 'ㅓ': ['ノ'], 'ㅕ': ['ニョ'], 'ㅗ': ['ノ'], 'ㅛ': ['ニョ'], 'ㅜ': ['ヌ'], 'ㅠ': ['ニュ'], 'ㅡ': ['ヌ'], 'ㅣ': ['ニ'] },
    'ㄷ': { 'ㅏ': ['タ', 'ダ'], 'ㅑ': ['チャ', 'ヂャ'], 'ㅓ': ['ト', 'ド'], 'ㅕ': ['チョ', 'ヂョ'], 'ㅗ': ['ト', 'ド'], 'ㅛ': ['チョ', 'ヂョ'], 'ㅜ': ['トゥ', 'ドゥ'], 'ㅠ': ['チュ', 'ヂュ'], 'ㅡ': ['トゥ', 'ドゥ'], 'ㅣ': ['ティ', 'ディ'] },
    'ㄹ': { 'ㅏ': ['ラ'], 'ㅑ': ['リャ'], 'ㅓ': ['ロ'], 'ㅕ': ['リョ'], 'ㅗ': ['ロ'], 'ㅛ': ['リョ'], 'ㅜ': ['ル'], 'ㅠ': ['リュ'], 'ㅡ': ['ル'], 'ㅣ': ['リ'] },
    'ㅁ': { 'ㅏ': ['マ'], 'ㅑ': ['ミャ'], 'ㅓ': ['モ'], 'ㅕ': ['ミョ'], 'ㅗ': ['モ'], 'ㅛ': ['ミョ'], 'ㅜ': ['ム'], 'ㅠ': ['ミュ'], 'ㅡ': ['ム'], 'ㅣ': ['ミ'] },
    'ㅂ': { 'ㅏ': ['パ', 'バ'], 'ㅑ': ['ピャ', 'ビャ'], 'ㅓ': ['ポ', 'ボ'], 'ㅕ': ['ピョ', 'ビョ'], 'ㅗ': ['ポ', 'ボ'], 'ㅛ': ['ピョ', 'ビョ'], 'ㅜ': ['プ', 'ブ'], 'ㅠ': ['ピュ', 'ビュ'], 'ㅡ': ['プ', 'ブ'], 'ㅣ': ['ピ', 'ビ'] },
    'ㅅ': { 'ㅏ': ['サ'], 'ㅑ': ['シャ'], 'ㅓ': ['ソ'], 'ㅕ': ['ショ'], 'ㅗ': ['ソ'], 'ㅛ': ['ショ'], 'ㅜ': ['ス'], 'ㅠ': ['シュ'], 'ㅡ': ['ス'], 'ㅣ': ['シ'] },
    'ㅈ': { 'ㅏ': ['ジャ', 'チャ'], 'ㅑ': ['ジャ', 'チャ'], 'ㅓ': ['ジョ', 'チョ'], 'ㅕ': ['ジョ', 'チョ'], 'ㅗ': ['ジョ', 'チョ'], 'ㅛ': ['ジョ', 'チョ'], 'ㅜ': ['ジュ', 'チュ'], 'ㅠ': ['ジュ', 'チュ'], 'ㅡ': ['ジュ', 'チュ'], 'ㅣ': ['ジ', 'チ'] },
